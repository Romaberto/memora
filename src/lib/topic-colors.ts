 ...
 * Topic color map.
 *
 * The editorial / monogram direction was rejected by product because the
 * two-letter tiles were hard to scan at a glance. We're back to the
 * emoji + colored-pastel treatment from earlier, which matches the landing
 * page's colorful convention and is easier to pattern-match visually.
 *
 * `getTopicMonogram()` is still exported in case we want to fall back to
 * initials when a topic has no emoji.
 */
...
export const COLOR_MAP: Record<string, TopicTile> = {
...
export function getTopicColors(color: string | null): TopicTile {
  return COLOR_MAP[color ?? ""] ?? DEFAULT_COLORS;
}

export function getTopicTile(color: string | null): TopicTile {
  return getTopicColors(color);
}

/** Fallback for topics that don't have an emoji defined. */
export function getTopicMonogram(name: string): string {
  if (!name) return "··";
